...
export default function Accounts() {
  ...
  const setCreateAcc = (accountGroup: AccountGroup) => {
    setEditAccount(undefined);
  ...
  const setEditAcc = (account?: Account) => {
    ...
    let ag = accountGroups.accountGroups?.find((element) => element.id == account?.account_group_id);
    setAccountGroup(ag);
    setDialogOpen(true);
  };
  return (
        ...
        <div className="ml-auto flex items-center gap-2">
          <DropdownMenu>
            ...
            <DropdownMenuContent align="end">
              {accountGroups.accountGroups?.map((element) => <DropdownMenuCheckboxItem key={"transaction-type-dropdown" + element.id} onClick={() => setCreateAcc(element)}>
                {element.name}
              </DropdownMenuCheckboxItem>)}
            </DropdownMenuContent>
          </DropdownMenu>
